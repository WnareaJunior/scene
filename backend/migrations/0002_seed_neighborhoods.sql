-- 0002_seed_neighborhoods — NYC location dictionary for search stage 2.
--
-- Same rows as src/search/sql/002_seed_neighborhoods.sql (kept there as the
-- annotated reference for coordinates, radii and alias rules). Idempotent on
-- `slug`: re-running updates rather than duplicating. Data, not schema, but it
-- is required for the search pipeline to parse place names, so it ships with
-- the schema.
